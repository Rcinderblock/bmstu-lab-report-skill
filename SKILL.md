...
- For coursework DOCX, the preferred contents block is a native Word TOC content control: `w:sdt` with `docPartGallery="Table of Contents"`, visible heading `СОДЕРЖАНИЕ`, TOC field code such as `TOC \o "1-3" \h \z \u`, clickable `w:hyperlink` entries, and page numbers produced through `PAGEREF` fields. Do not replace a user-corrected native TOC with a static imitation. Do not set `w:updateFields` unless the user accepts Word asking to update fields on open.
...
- Do not use colored shading/fills in coursework РПЗ tables by default. Table headers should be unfilled/white with black text and standard borders unless the provided department template explicitly uses another style.
...
- Do not add a blanket page break before every table. Tables should normally appear after their first mention or on the next page; move a whole table to a fresh page only when render inspection shows that the caption/table pair would otherwise split badly or leave only a few rows on a page.
- Never place two non-text elements back-to-back. Before every table, figure, or listing add a short lead-in paragraph that names why the element is being introduced; after every table, figure, or listing add an interpretation paragraph that states what the reader should take from it. This includes transitions between a table and a figure.
- During render review, look for ugly page starts: a page must not begin with a lonely table/listing continuation fragment, a single trailing row, a detached caption, or a short orphaned paragraph that belongs visually to the previous page. Fix these by moving the related paragraph/caption/table together, splitting the table deliberately, or lightly rebalancing nearby prose; do not introduce large blank areas as a workaround.
...
- В DOCX для РПЗ заголовки должны быть настоящими стилями Word `Heading 1`, `Heading 2` и далее, а не абзацами `Normal` с похожим прямым форматированием. После генерации проверяй стилевой аудит: структурные и нумерованные заголовки имеют heading-style, Times New Roman 14 pt, черный цвет и полужирное начертание.
...
When making low-level OOXML edits, do not reserialize the whole `word/document.xml` with a generic XML serializer unless all original namespace prefixes and compatibility declarations are preserved. In particular, `mc:Ignorable` must not reference prefixes such as `w14`, `w15`, `w16*`, or `wp14` unless the matching `xmlns:*` declarations are still present. After OOXML surgery, validate by parsing all XML parts, running `unzip -t`, checking for missing `mc:Ignorable` prefixes, and opening/rendering the file; a ZIP-valid DOCX can still trigger Microsoft Word's unreadable-content repair prompt.
